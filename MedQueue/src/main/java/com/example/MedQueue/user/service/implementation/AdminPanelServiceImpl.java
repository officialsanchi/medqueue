...
public class AdminPanelServiceImpl implements AdminPanelService {
    ...
    public SystemSetting getSettings() {
        return settingRepository.findAll().stream().findFirst().orElse(new SystemSetting());
    }

    public SystemSetting updateSettings(SystemSetting newSettings) {
        SystemSetting setting = getSettings();
        setting.setMaxQueueSize(newSettings.getMaxQueueSize());
        setting.setEmergencyModeEnabled(newSettings.isEmergencyModeEnabled());
